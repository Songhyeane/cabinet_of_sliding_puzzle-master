...
import numpy as np
import time
from app.alogorism1.game_state import GameState
...
class Solver():
    ...
    def solve_a_star(self):
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state[6] == self.__goal_state.flatten().tolist()[6]:
    ...
    def solve_bfs(self):
        ...
        while nodes.qsize() and epochs <= self.__max_iter:
            ...
            if cur_state == self.__goal_state.flatten().tolist():
                self.__summary = str("BFS took " + str(cur_node.get_level()) + " steps to get from initial state to the desired goal, visited total of " + str(epochs) + " nodes, and took around " + str(np.round(time.perf_counter() - start_time, 4)) + " seconds to reach the desired solution.")
                while cur_node.get_parent():
                    self.__path.append(cur_node)
                    cur_node = cur_node.get_parent()
                break
